...
func InitializeContribIssueSnapshotURLValidationMatrixScenario(ctx *godog.ScenarioContext) {
	state := &scenarioState{}
...
func TestContribIssueSnapshotURLValidationMatrixFeatures(t *testing.T) {
	...
	suite := godog.TestSuite{
		ScenarioInitializer: InitializeContribIssueSnapshotURLValidationMatrixScenario,
		Options: &godog.Options{
			Format:   "pretty",
			Paths:    []string{filepath.Join(intendRepoRoot, "features", "contrib-issue-snapshot-url-validation-matrix.feature")},
			Strict:   true,
			TestingT: t,
		},
	}
