...
func InitializeAmendReportingScenario(ctx *godog.ScenarioContext) {
	state := &scenarioState{}
...
func TestAmendReportingFeatures(t *testing.T) {
	...
	suite := godog.TestSuite{
		ScenarioInitializer: InitializeAmendReportingScenario,
		Options: &godog.Options{
			Format:   "pretty",
			Paths:    []string{filepath.Join(intendRepoRoot, "features", "amend-reporting.feature")},
			Strict:   true,
			TestingT: t,
		},
	}
