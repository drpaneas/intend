...
func InitializeContribTraceIssueMetadataValidationMatrixScenario(ctx *godog.ScenarioContext) {
	state := &scenarioState{}
...
func TestContribTraceIssueMetadataValidationMatrixFeatures(t *testing.T) {
	...
	suite := godog.TestSuite{
		ScenarioInitializer: InitializeContribTraceIssueMetadataValidationMatrixScenario,
		Options: &godog.Options{
			Format:   "pretty",
			Paths:    []string{filepath.Join(intendRepoRoot, "features", "contrib-trace-issue-metadata-validation-matrix.feature")},
			Strict:   true,
			TestingT: t,
		},
	}
