...
func InitializeContribIssueSnapshotDataValidationMatrixScenario(ctx *godog.ScenarioContext) {
	state := &scenarioState{}
...
func TestContribIssueSnapshotDataValidationMatrixFeatures(t *testing.T) {
	...
	suite := godog.TestSuite{
		ScenarioInitializer: InitializeContribIssueSnapshotDataValidationMatrixScenario,
		Options: &godog.Options{
			Format:   "pretty",
			Paths:    []string{filepath.Join(intendRepoRoot, "features", "contrib-issue-snapshot-data-validation-matrix.feature")},
			Strict:   true,
			TestingT: t,
		},
	}
