...
func InitializeTracePathSafetyValidationMatrixScenario(ctx *godog.ScenarioContext) {
	state := &scenarioState{}
...
func TestTracePathSafetyValidationMatrixFeatures(t *testing.T) {
	...
	suite := godog.TestSuite{
		ScenarioInitializer: InitializeTracePathSafetyValidationMatrixScenario,
		Options: &godog.Options{
			Format:   "pretty",
			Paths:    []string{filepath.Join(intendRepoRoot, "features", "trace-path-safety-validation-matrix.feature")},
			Strict:   true,
			TestingT: t,
		},
	}
