...
func InitializeContribIssueSnapshotIdentityValidationMatrixScenario(ctx *godog.ScenarioContext) {
	state := &scenarioState{}
...
func TestContribIssueSnapshotIdentityValidationMatrixFeatures(t *testing.T) {
	...
	suite := godog.TestSuite{
		ScenarioInitializer: InitializeContribIssueSnapshotIdentityValidationMatrixScenario,
		Options: &godog.Options{
			Format:   "pretty",
			Paths:    []string{filepath.Join(intendRepoRoot, "features", "contrib-issue-snapshot-identity-validation-matrix.feature")},
			Strict:   true,
			TestingT: t,
		},
	}
